...
可用命令行或者ui面板进行创建，具体的创建方式及详细说明，自行百度
...
- vue-router: 页面路由，用于做单页面应用(SPA)。
- vuex: 数据状态管理。
- vue-navigation: 记录路由并缓存页面，像原生APP导航一样。
- vconsole: 移动端调试利器
- axios: 一个很好的ajax封装集
- fastclick: 为了能够立即响应用户的点击事件，解决移动端click 300ms延迟，才有了FastClick。
- webpack: 用于搭建前端工程。
