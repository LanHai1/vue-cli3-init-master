...
let BASE = {
  send(obj) {
    ...
    axios.request({
      ...
      // 处理非正常业务数据
      let handleErrorResponse = (res, obj) => {
...
// 将data转换为FormData包装
let handleData2FormData = data => {
  // 判断是不是文件
  let isFile = (file) => file instanceof File;
...
// 对应所有get请求
const _get = obj => {
  let config = defaultHandleConfig(obj);
  config.params = obj.params;
  BASE.send(config);
};

// 对应spring mvc中的@RequestPart，支持文件上传
const _postFormData = obj => {
  let config = defaultHandleConfig(obj);
